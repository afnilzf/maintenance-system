...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4>Jadwal Perawatan Preventif</h4>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if(Auth::user()->role === 'kepala_bengkel' && !$schedule->approved_by_head)
                        <form action="{{ route('preventive-schedules.approve', $schedule->id) }}" method="POST" style="display:inline">
                            @csrf
                            <button class="btn btn-success btn-sm">Setujui</button>
                        </form>
                        @elseif(Auth::user()->role === 'kepala_jurusan' && $schedule->approved_by_head && !$schedule->approved_by_department)
                        <form action="{{ route('preventive-schedules.approve', $schedule->id) }}" method="POST" style="display:inline">
                            @csrf
                            <button class="btn btn-success btn-sm">Setujui</button>
                        </form>
                        ...
                        @if(Auth::user()->role !== 'plp' && $schedule->approval_status === 'pending')
                        <form action="{{ route('preventive-schedules.reject', $schedule->id) }}" method="POST" style="display:inline">
                            @csrf
